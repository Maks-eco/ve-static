...
function generPyramidNewLayer(arrPyr, step, colorInterval) {
  ...
  for (let i = 0; i < arrPyr[base].length; i++) {
    ...
    for (let j = i + 1; j < arrPyr[base].length; j++) {
      // find min length
      if (arrPyr[base][j].sta !== "f") continue;
      if (Math.abs(arrPyr[base][i].val - arrPyr[base][j].val) > colorInterval)
        continue;
    ...
    if (setFnd) {
      // append fusion two nearby points (center point)
      const [centerX, centerY] = centerTwoPoints(
        arrPyr[base][i],
        arrPyr[base][minIndex]
      );
      arrPyr[curr].push({
        x: centerX,
        y: centerY,
...
async function formingPyramidRefs(initArr) {
