...
function drawCanvasLoop(data, createCanvas) {
  const feDataImgs = []
  const { canvas, width, height } = createCanvasElement(CONST_DIMENS * scale + 20, CONST_DIMENS * scale + 20, createCanvas)
...
function resizeCanvasImage(img, width, height, createCanvas) {
  const canvasNew = createCanvasElement(CONST_DIMENS, CONST_DIMENS, createCanvas).canvas
